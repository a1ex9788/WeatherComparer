...
public class OpenWeatherForecast extends WeatherForecast {
    ...
    private List<DayForecast> convertToDailyStandard(
            OpenWeatherCompleteForecast openWeatherCompleteForecast) {
        List<DayForecast> dailyForecast = new ArrayList();

        // The first element is today's forecast so it has to be skipped.
        for (OpenWeatherCompleteForecast.OpenWeatherDayForecast openWeatherDayForecast :
                openWeatherCompleteForecast.daily.subList(
                1, openWeatherCompleteForecast.daily.size())) {
            dailyForecast.add(new DayForecast(openWeatherDayForecast.dt == null ? null
                    : UnitsConverter.unixUtcToDate(openWeatherDayForecast.dt),
                    openWeatherDayForecast.weather == null
                            || openWeatherDayForecast.weather.get(0) == null
                            || openWeatherDayForecast.weather.get(0).id == null ? null
                            : UnitsConverter.openWeatherConditionToStandard(
                                    openWeatherDayForecast.weather.get(0).id),
                    openWeatherDayForecast.temp == null ? null
                            : (openWeatherDayForecast.temp.max + openWeatherDayForecast.temp.min)
                                    / 2.0,
                    ...
                    openWeatherDayForecast.feels_like == null ? null
                            : openWeatherDayForecast.feels_like.day, openWeatherDayForecast.pop,
                    openWeatherDayForecast.humidity, openWeatherDayForecast.clouds,
                    openWeatherDayForecast.wind_speed == null ? null
                            : UnitsConverter.metersPerSecondToKilometersPerHour(
                                    openWeatherDayForecast.wind_speed),
                    openWeatherDayForecast.pressure, openWeatherDayForecast.uvi,
                    openWeatherDayForecast.sunrise == null ? null
                            : UnitsConverter.unixUtcToDate(openWeatherDayForecast.sunrise),
                    openWeatherDayForecast.sunset == null ? null
    ...
    private List<HourForecast> convertToHourlyStandard(
            OpenWeatherCompleteForecast openWeatherCompleteForecast) {
        ...
        for (OpenWeatherCompleteForecast.OpenWeatherHourForecast openWeatherHourForecast :
                openWeatherCompleteForecast.hourly.subList(
                1, openWeatherCompleteForecast.hourly.size())) {
            hourlyForecast.add(new HourForecast(openWeatherHourForecast.dt == null ? null
                    : UnitsConverter.unixUtcToDate(openWeatherHourForecast.dt),
                    openWeatherHourForecast.weather == null
                            || openWeatherHourForecast.weather.get(0) == null
                            || openWeatherHourForecast.weather.get(0).id == null ? null
                            : UnitsConverter.openWeatherConditionToStandard(
                                    openWeatherHourForecast.weather.get(0).id),
                    openWeatherHourForecast.temp, openWeatherHourForecast.feels_like,
                    openWeatherHourForecast.pop, openWeatherHourForecast.humidity,
                    openWeatherHourForecast.clouds,
                    openWeatherHourForecast.wind_speed == null ? null
                            : UnitsConverter.metersPerSecondToKilometersPerHour(
                                    openWeatherHourForecast.wind_speed),
                    openWeatherHourForecast.pressure, openWeatherHourForecast.uvi));
        }
